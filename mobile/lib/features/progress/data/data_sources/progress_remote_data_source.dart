...
/// Remote data source for progress-related endpoints.
class ProgressRemoteDataSource {
  ...
  ///
  /// GET /api/v1/customers/customers/{id}/measurements/
  Future<List<BodyMeasurement>> getMeasurements(int customerId) async {
    try {
      final response = await _dio.get(
        AppConstants.customerMeasurementsEndpoint.replaceAll('{id}', '$customerId'),
      );
  ...
  ///
  /// GET /api/v1/customers/customers/{id}/fitness-goals/
  Future<List<FitnessGoal>> getFitnessGoals(int customerId) async {
    try {
      final response = await _dio.get(
        AppConstants.customerFitnessGoalsEndpoint.replaceAll('{id}', '$customerId'),
      );
      final results = _extractResults(response.data);
      return results
