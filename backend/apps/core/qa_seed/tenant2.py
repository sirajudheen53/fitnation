...
EXERCISES = [
    ...
    ("PVC Mobility Flow", "Mobility"),
    ("Hip Hinge Warmup", "Mobility"),
    ("Lat Stretch", "Flexibility"),
    ("Kneeling Quad Stretch", "Flexibility"),
]
...
def _seed_branches(tenant: Tenant) -> list[Branch]:
    ...
    specs = [
        (
            "Secunderabad",
            "flagship",
            "42, Rasoolpura Main Road",
            "Secunderabad",
            "500003",
            17.4375,
            78.4986,
            "+914012345678",
            "secunderabad@ironhouse.test",
            "05:00",
            "23:00",
            True,
        ),
        (
            "Gachibowli",
            "standard",
            "78, Financial District",
            "Gachibowli",
            "500032",
            17.4401,
            78.3489,
            "+914045678901",
            "gachibowli@ironhouse.test",
            "05:30",
            "22:30",
            False,
        ),
    ]
    for name, btype, line1, line2, postal, lat, lng, phone, email, opens, closes, hq in specs:
    ...
    for branch in branches:
        for amenity in ["Steam Room", "CrossFit Rig", "Parking", "Whey Bar", "Showers"]:
            BranchAmenity.objects.get_or_create(branch=branch, name=amenity, defaults={"is_available": True})
    return branches
...
def _seed_products(tenant: Tenant) -> list[Product]:
    ...
    for name, cat, sku, price, compare, stock in items:
        ...
        Inventory.objects.get_or_create(
            tenant=tenant,
            product=product,
            defaults=dict(stock_quantity=stock, low_stock_threshold=5, track_inventory=True),
        )
...
def _seed_staff(tenant: Tenant, reset_passwords: bool) -> list:
    ...
    for email, fn, ln, spec, yrs in specs:
        ...
        trainer = ensure_trainer(
            user,
            specialization=spec,
            bio=f"{fn} coaches {spec} at IronHouse.",
            is_active=True,
            certifications=["CrossFit L1", "ACE"],
...
def seed_base(reset_passwords: bool = True, echo=print) -> tuple:
    ...
    echo(f"  IronHouse: tenant + {len(branches)} branches + catalogs + " f"{len(trainers)} trainers + staff")
...
def _seed_customers(tenant: Tenant, branches: list[Branch]) -> list[Customer]:
    ...
    for name, lname, email, gender, hgt, wgt in CUSTOMERS:
        ...
        user = ensure_user(
            ...
            last_name=lname,
            role="customer",
            password=PASSWORDS["tenant2_customers"],
            tenant=tenant,
            phone=f"+91{random.randint(7000000000, 9999999999)}",
            is_staff=False,
        )
        customer = Customer.objects.create(
            ...
            address_street=(
                ...
                f"{random.choice(['Banjara Hills', 'Madhapur', 'Kondapur', 'Gachibowli'])}"
            ),
        ...
        FitnessGoal.objects.create(
            ...
            is_active=True,
            status="active",
            target_value=random.choice([5, 10, 15]),
        ...
        BodyMeasurement.objects.create(
            ...
            chest_cm=round(random.uniform(88, 112), 1),
            waist_cm=round(random.uniform(70, 102), 1),
            hips_cm=round(random.uniform(88, 106), 1),
            biceps_cm=round(random.uniform(28, 39), 1),
            thighs_cm=round(random.uniform(50, 66), 1),
...
def _seed_memberships(tenant: Tenant, customers: list[Customer]) -> None:
    ...
    for name, ptype, price, dur, desc in pdata:
        plan, _ = MembershipPlan.objects.get_or_create(
            tenant=tenant,
            name=name,
            defaults=dict(plan_type=ptype, price=price, duration_days=dur, description=desc, is_active=True),
        )
        plans.append(plan)
    Coupon.objects.get_or_create(
        ...
        defaults=dict(
            ...
            valid_to=dt.date.today() + dt.timedelta(days=45),
            is_active=True,
        ),
    )
...
def _seed_diet(tenant: Tenant, customers: list[Customer], echo) -> None:
    ...
    for name, goal, cal, pr, cr, fr, dur in plans:
        plan = DietPlan.objects.create(
            tenant=tenant,
            name=name,
            description=f"{name} for IronHouse members.",
            goal=goal,
            daily_calories=cal,
            protein_ratio=pr,
            carb_ratio=cr,
        ...
        for d in range(1, 8):
            ...
            for meal_type, group, _frac in [
                ("breakfast", "grains", 0.3),
                ("morning_snack", "fruit", 0.1),
                ("lunch", "protein", 0.3),
                ("evening_snack", "snack", 0.1),
                ("dinner", "protein", 0.2),
            ]:
...
def _seed_workouts(tenant: Tenant, customers: list[Customer], echo) -> None:
    ...
    for name, goal, diff, weeks in plans:
        plan = WorkoutPlan.objects.create(
            tenant=tenant,
            name=name,
            description=f"{name} at IronHouse.",
            goal=goal,
            difficulty=diff,
        ...
        for i, day in enumerate(["monday", "wednesday", "friday"]):
            wd = WorkoutDay.objects.create(
                tenant=tenant, workout_plan=plan, day_of_week=day, day_number=i + 1, focus=focus_cycle[i % 4], notes=""
            )
            for order in range(1, 6):
                ...
                WorkoutExercise.objects.create(
                    ...
                    reps=random.choice(["8-12", "6-8", "12-15"]),
    ...
    for customer in random.sample(customers, min(4, len(customers))):
        ...
        for product in random.sample(products, random.randint(1, 2)):
            CartItem.objects.create(
                tenant=tenant, cart=cart, product=product, quantity=random.randint(1, 2), unit_price=product.price
            )
...
def seed_realistic(reset_passwords: bool = True, echo=print) -> None:
    ...
    echo(f"  IronHouse realistic: {len(customers)} customers, " "memberships/diet/workouts/ops seeded")
