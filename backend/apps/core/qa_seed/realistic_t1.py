...
def seed_staff(tenant, reset_passwords: bool = True, echo=print) -> list[Trainer]:
    ...
    ensure_user(
        email="owner@fitnation.test", first_name="Rahul", last_name="Sharma",
        role="gym_owner", password=password, tenant=tenant,
        phone="+919812345670", reset_passwords=reset_passwords,
    )
    ensure_user(
        email="manager@fitnation.test", first_name="Priya", last_name="Nair",
        role="manager", password=password, tenant=tenant,
        phone="+919812345671", reset_passwords=reset_passwords,
    )
    ensure_user(
        email="dietitian@fitnation.test", first_name="Ananya", last_name="Iyer",
        role="dietitian", password=password, tenant=tenant,
        phone="+919812345672", reset_passwords=reset_passwords,
    )
...
def _seed_customers(tenant, branch) -> list[Customer]:
    ...
    for name, lname, email, gender, hgt, wgt in CUSTOMERS:
        ...
        user = ensure_user(
            ...
            password=PASSWORDS["tenant1_customers"], tenant=tenant,
        ...
        customer = Customer.objects.create(
            ...
            address_street=f"{random.randint(1, 300)}, {random.choice(['MG Road', 'Linking Road', 'Banjara Hills', 'Indiranagar', 'Koramangala'])}",
        ...
        HealthProfile.objects.create(
            ...
            injuries="", current_injuries=[], past_injuries=[],
            medical_info={}, medical_conditions=[], allergies=[],
            food_allergies=[], medications=[], dietary_restrictions=[],
        )
        goal_type, _notes, val, unit = random.choice(GOAL_POOL)
        FitnessGoal.objects.create(
            tenant=tenant, customer=customer, goal_type=goal_type,
            is_active=True, status="active", target_value=val,
            target_unit=unit, target_date=dt.date.today() + dt.timedelta(days=random.randint(60, 180)),
...
def _seed_plans(tenant, customers) -> None:
    ...
    for name, ptype, price, days, desc in plan_specs:
        plan, _ = MembershipPlan.objects.get_or_create(
            ...
            defaults=dict(plan_type=ptype, price=price, duration_days=days,
                          description=desc, is_active=True),
        )
        plans.append(plan)

    Coupon.objects.get_or_create(
        tenant=tenant, code="FITNATION50",
        defaults=dict(discount_percent=10, max_uses=100, used_count=5,
                      valid_from=dt.date.today() - dt.timedelta(days=30),
                      valid_to=dt.date.today() + dt.timedelta(days=60), is_active=True),
    )
...
def _seed_diet(tenant, echo) -> None:
    ...
    for name, goal, cal, pr, cr, fr, dur in plans_data:
        plan = DietPlan.objects.create(
            tenant=tenant, name=name, description=f"{name} tailored for Indian diet.",
        ...
        for d in range(1, 8):
            ...
            for meal_type, group, _frac in [
                ("breakfast", "grains", 0.30), ("morning_snack", "fruit", 0.10),
                ("lunch", "protein", 0.30), ("evening_snack", "snack", 0.10),
                ("dinner", "protein", 0.20),
            ]:
...
def _seed_workouts(tenant, customers, echo) -> None:
    ...
    for name, goal, diff, weeks in plans_data:
        plan = WorkoutPlan.objects.create(
            tenant=tenant, name=name, description=f"{name} program.",
        ...
        for i, day in enumerate(["monday", "tuesday", "thursday", "friday"]):
            ...
            for order in range(1, 6):
                ...
                WorkoutExercise.objects.create(
                    ...
                    rpe=random.randint(6, 9), notes="", order=order,
                )
    ...
    for i, customer in enumerate(customers):
        ...
        if not TrainerAssignment.objects.filter(customer=customer).exists():
            TrainerAssignment.objects.create(
                tenant=tenant, trainer=trainer, customer=customer, branch=branch,
    ...
    for customer in random.sample(customers, min(14, len(customers))):
        for d in range(0, 14, 2):
            ...
            AttendanceRecord.objects.create(
                tenant=tenant, customer=customer, branch=branch,
...
def seed_realistic(tenant, branch, trainers, echo=print) -> None:
    ...
    echo(f"  tenant-1 realistic: {len(created)} new customers, "
