...
class AdapterRegistryTest(AccessTestBase):
    ...
    def test_all_vendors_resolve_to_adapters(self) -> None:
        """Every vendor (native or stub) resolves to a BiometricAdapter."""
        for vendor in ("hikvision", "zkteco", "essl", "matrix"):
...
class SyncServiceTest(AllowListTestBase):
    ...
    def test_sync_device_unreachable_reports_failure(self) -> None:
        """Unreachable devices report a clear failure instead of raising."""
        from apps.access.models import BiometricCredential
        ...
        self.device.save()
        result = sync_device(self.device)  # hikvision real adapter, connection fails
        assert result["synced"] is False
