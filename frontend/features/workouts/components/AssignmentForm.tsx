...
import { Users, Dumbbell, Calendar, StickyNote } from "lucide-react";
import { Button, Input, Alert } from "@/components/ui";
import type { Customer } from "@/types/customer";
import type { WorkoutAssignmentFormData, WorkoutPlan } from "@/types/workout";
...
interface AssignmentFormProps {
  ...
  plans: WorkoutPlan[];
  submitLabel?: string;
  onSubmit: (data: WorkoutAssignmentFormData) => void | Promise<void>;
  error?: unknown;
  loading?: boolean;
}
...
export function AssignmentForm({
  ...
  const [workoutPlan, setWorkoutPlan] = useState<number | "">("");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (customer === "" || workoutPlan === "" || !startDate) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
      ...
      <div className="space-y-1.5">
        <label htmlFor="workout_plan" className="block text-sm font-medium text-gray-700">
          Workout plan
        </label>
        <div className="relative">
          ...
          <select
            id="workout_plan"
            value={workoutPlan}
            onChange={(e) => setWorkoutPlan(e.target.value ? Number(e.target.value) : "")}
            className="block w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-3 text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
          >
